...
export default function InvalidatePage() {
  ...
  useEffect(() => {
    ...
    const call = token ? api.get(`/auth/invalidate?token=${encodeURIComponent(token)}`) : Promise.resolve();
      ...
      <Card className="w-full max-w-md bg-[#1a2235] border-[#1e2d45]">
        <CardHeader className="text-center space-y-3">
          <div className="flex justify-center">
            {done ? <ShieldOff className="h-14 w-14 text-red-400" /> : <Loader2 className="h-14 w-14 animate-spin text-amber-500" />}
          </div>
          <CardTitle className="text-[#f1f5f9]">Session Terminated</CardTitle>
          <CardDescription className="text-[#94a3b8]">
            The suspicious session has been ended. If this keeps happening, your email account may be compromised. Contact the portal administrator.
          </CardDescription>
        </CardHeader>
