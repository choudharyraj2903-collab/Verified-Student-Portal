...
function extractRawToken(value: string): string {
  try {
    const nestedURL = new URL(value);
    return nestedURL.searchParams.get("token") ?? value;
  } catch {
    return value;
  }
}
...
export default function VerifyPage() {
  ...
  const [hasToken] = useState(() => Boolean(token));
  ...
  useEffect(() => {
    ...
    window.location.href = `${API_URL}/auth/verify?token=${encodeURIComponent(rawToken)}`;
  }, [router, token]);
      ...
      <Card className="w-full max-w-md bg-[#1a2235] border-[#1e2d45]">
        <CardHeader className="text-center">
          <CardTitle className="text-[#f1f5f9]">
            {hasToken ? "Verifying your login link..." : "Verification Failed"}
          </CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col items-center gap-4 pb-6">
          {hasToken && <Loader2 className="h-10 w-10 animate-spin text-amber-500" />}
          {!hasToken && (
            <>
