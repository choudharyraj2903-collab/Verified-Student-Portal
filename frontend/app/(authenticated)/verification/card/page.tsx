...
interface VerifiedCard {
  ...
  verified_records: Record<string, VerifiedRecord[]>; // key = council_id (UUID)
...
export default function VerifiedCardPage() {
  ...
  useEffect(() => {
    const userId = searchParams.get("userId") ?? profile?.profile.user_id;
    if (!userId) { setLoading(false); return; }
    Promise.all([
      api.get<VerifiedCard>(`/verification/card/${userId}`),
      api.get<Council[]>("/councils"),
    ]).then(([cardRes, councilRes]) => {
      if (cardRes.success) setCard(cardRes.data);
      if (councilRes.success) setCouncils(councilRes.data ?? []);
    }).finally(() => setLoading(false));
  }, [searchParams, profile]);

  if (loading) return <div className="p-6"><LoadingSpinner /></div>;
  if (!card) return <div className="flex items-center justify-center min-h-screen"><p className="text-[#94a3b8]">Card not available.</p></div>;

  const { student, verified_records } = card;
  const initials = student.full_name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();

  // Resolve council UUID → name/code
  function councilLabel(id: string) {
    const c = councils.find((c) => c.id === id);
    return c ? `${c.name} (${c.code})` : id.slice(0, 8) + "…";
  }

  const entries = Object.entries(verified_records ?? {}).filter(([, recs]) =>
    ...
    <div className="p-6">
      <div className="flex justify-end mb-4 print:hidden">
        <button onClick={() => window.print()}
          className="inline-flex items-center gap-1.5 rounded-lg border border-[#1e2d45] text-[#94a3b8] hover:bg-[#1e2d45] hover:text-[#f1f5f9] text-sm px-3 h-8 transition-colors">
      ...
      <div className="max-w-2xl mx-auto rounded-2xl border border-[#1e2d45] bg-[#1a2235] overflow-hidden shadow-2xl">
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          ) : entries.map(([councilId, records]) => {
            const approved = records.filter((r) => r.status === "APPROVED");
            return (
              <div key={councilId}>
                <p className="text-xs font-semibold uppercase tracking-widest text-amber-500 mb-3">{councilLabel(councilId)}</p>
                <div className="space-y-4">
                  {approved.map((rec) => (
                    <div key={rec.id} className="rounded-lg border border-[#1e2d45] bg-[#111827] p-4">
                      ...
                      {rec.remarks && <p className="mt-2 text-xs text-[#94a3b8] italic">"{rec.remarks}"</p>}
                    </div>
                  ))}
                </div>
