...
func Generate(r *http.Request, cfg *config.AppConfig) (*MagicTokenResults, error) {
    ...
    fp := utils.ExtractFingerprint(r) // corrected name
    uaHash := utils.Hash256String(fp.UserAgent)

    expiresAt := time.Now().UTC().Add(time.Duration(cfg.MagicLinkExpiryMinutes) * time.Minute)
...
func Store(ctx context.Context, pool *pgxpool.Pool, userID string, result *MagicTokenResults) error {
    _, err := pool.Exec(ctx,
        `INSERT INTO magic_tokens (user_id, token_hash, ua_hash, expires_at, is_used)
...
func Verify(ctx context.Context, pool *pgxpool.Pool, rawToken string, r *http.Request) (*MagicTokenResults, error) {
    ...
    row := pool.QueryRow(ctx,
        `SELECT id, user_id, token_hash, ua_hash, expires_at, is_used, created_at
    ...
    if token.UAHash != utils.Hash256String(utils.ExtractFingerprint(r).UserAgent) {
